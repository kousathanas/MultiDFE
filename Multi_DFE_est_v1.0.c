...
n_e=calculate_ne(N1,n2,t);
  ...
  if (f0<0.0||f0>1.0){lik=undefined;goto End;}
...
/*select selection mode*/
  switch (selmode)
      ...
      for(i = 1; i <= nspikes; i++)
	{
	  ...
	  if (spikes_vec[i]>0.0){lik=undefined;goto End;}
 	  if (spikes_vec[i-1]<spikes_vec[i]){lik=undefined;goto End;} 
 ...
 /*calculate FV0*/
      checker=calculate_FV(selmode,N1,n2,t,0.0,0.0,0.0,0.0,f0,FV0);if (checker==undefined){lik=undefined;goto End;}    
...
/*calculate FVSs*/
      for(i = 1; i <= nspikes; i++)
	{
      ...
      for (i=1;i<=nspikes;i++){
	for (j=0;j<n2d;j++) FVS[j]+=prob_vec[i] * FVSX[i][j];       
      }  
...
double  likelihood_function_neutral (const gsl_vector *v, void *params) 
{
  ...
  f0 = gsl_vector_get(v, 0);
  if (!conpop){t=gsl_vector_get(v, 1);}else{t=100;}
...
//monitorinput();
	  ...
	  for (i = 1; i < number_s ; i++)
	    {
	      if (s_val[i+1]<s_val[i]){check=1;}else{check=0;break;}
	    }
	}//while(check==0;)
...
//monitorinput();
	while (diff_max>1e-5)
	{
	 ...
	 if (reps>4){break;}//no more than ten calls to the simplex
	}
...
double mean_s,double f0,double *mean_FV)
{
  ...
  if (!conpop)
      ...
      if ((t2_lower==undefined_int)||(t2_upper==undefined_int))
	{
      ...
      if (mean_s!=0.0){//selection different than 0  
	...
	for (i=0; i<n_s_evaluated; i++)
	  {
	    get_average_egf_vecs_1_and_2(s_evaluated_vec[i],n1, n2, t2_real, t2_lower,
					 ...
					 buffer_p2_t2_upper);
	    for (j=0; j<=n2d; j++) mean_FV[j] += egf_vec[j]*density_vec[i];
	  }  
      }
      ...
      if (s==0.0)
	{
	  get_average_egf_vecs_1_and_2(0.0,n1, n2, t2_real, t2_lower,
				       t2_upper, egf_vec, buffer_p1_t2_lower, buffer_p2_t2_lower, buffer_p1_t2_upper,
				       buffer_p2_t2_upper);
	  for (j=0; j<=n2d; j++) mean_FV[j] = egf_vec[j];
	}
...
int argc; char **argv;
{
  ...
  /*check N1*/
  if (N1==20||N1==100||N1==1000)
    {}else
    {
      printf("Please give only N1 values of 20,100,1000\n");
      return(0);
    }
  ...
  if (!conpop)
    {
      ...
      if (N2==0)/*launch N2 minimization*/
	{
